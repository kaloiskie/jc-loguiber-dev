...
import { ghFetchReadmeRaw } from './github-api'
...
function prepareReadmeHtml(markdown: string, owner: string, repo: string) {
...
function ReadmeModal({ owner, repo, token, onClose }: ReadmeModalProps) {
  ...
  useEffect(() => {
    ...
    ghFetchReadmeRaw(owner, repo, token, controller.signal)
      .then(async (response) => {
        if (!response.ok) throw new Error('README unavailable')
        const markdown = await response.text()
        setState({ status: 'ready', html: prepareReadmeHtml(markdown, owner, repo) })
      })
      .catch((error: unknown) => {
    ...
    <div className="readme-overlay" onMouseDown={onClose}>
      ...
      >
        <header className="readme-header">
          <div>
            <span>Repository document / Fig. R1</span>
        ...
        <div className="readme-layout">
          <aside className="readme-rail" aria-hidden="true">
            ...
            <span>Rendered from GitHub</span>
          ...
          <main className="readme-body">
            ...
            {state.status === 'error' && (
              <div className="readme-error">
                ...
                <p>The repository document could not be loaded from GitHub.</p>
                <a href={repositoryUrl} target="_blank" rel="noopener noreferrer">
